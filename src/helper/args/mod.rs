...
const CHECK_EMOTICON: &str = "(Â°ã­Â°)";
/// Emoticon for "found" message.
pub const FOUND_EMOTICON: &str = "\\(^ã®^)/";
/// Emoticon for "fail" message.
pub const FAIL_EMOTICON: &str = "(Ãï¹Ã)";
...
---
(Â°ã­Â°)  checking 'test help'
(Ãï¹Ã)      fail 'help' argument not found.
(o_O)      debug
stdout:
---
...
---",
    ...
    fn test_get_args_help() -> Result<()> {
        ...
        println!("{}", String::from_utf8_lossy(&output));
        assert_eq!(
            r"(Â°ã­Â°)  checking 'test -x'
(Ãï¹Ã)      fail '-x' argument not found.
(Â°ã­Â°)  checking 'test -V'
\(^ã®^)/ success '-V' argument found!
---
halp 0.1.0
